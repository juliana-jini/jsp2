...
<!-- 				<li>글 비밀번호</li><!-- 댓글 삭제 : 1)로그인한 사용자가 본인글 삭제 또는 2)글 비밀번호 사용				
				...
				<ul style="display: flex;">
					<li>
						...
						<!-- sessionScope.user는 session에 저장된 애트리뷰트 user를 가져옵니다. 
							 sessionScope을 생략할 수 있지만 request, application 등 다른 영역에 중복된 이름이 있을때를
							 고려하여 명시적으로 지정합니다.
						-->
	...
	//모달 입력 상자 닫기
	//let close = document.getElementsByClassName('close')[0];	//getElementsXXX이면 배열 리턴. 그 요소 중에 0번 인덱스
	...
	function deleteCmt(cmtidx,idx) {
		...
		if(yn) {
			//함수의 인자로 받은 2개의 값은 파라미터로 전달합니다. 추가된 파라미터 del이 있습니다.
